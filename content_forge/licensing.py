"""
Licensing and tier management for Content Forge.
...
PRODUCT_NAME = "content-forge"
TRIAL_DAYS = 30
LICENSE_STORE = Path.home() / ".content-forge" / "license.json"
...
def check_limit(feature, value=None):
    ...
    if feature == "max_words" and value is not None:
        max_val = limits.get("max_words")
        if max_val is not None and value > max_val:
            return False, (
                f"Free tier limited to {max_val} words. "
                f"Upgrade to Premium for unlimited: https://tirandev.gumroad.com"
            )

    if feature == "content_type" and value is not None:
        allowed = limits.get("content_types", [])
        if value not in allowed:
            return False, (
                f"Content type '{value}' requires Premium. "
                f"Free tier supports: {', '.join(allowed)}. "
                f"Upgrade: https://tirandev.gumroad.com"
            )

    if feature == "humanize":
        if not limits.get("humanize"):
            return False, (
                "AI humanization requires Premium. "
                "Upgrade: https://tirandev.gumroad.com"
            )

    if feature == "batch_generation":
        if not limits.get("batch_generation"):
            return False, (
